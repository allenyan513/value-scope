...
// Stage 1 (Y1–5): Analyst estimates, same as calculateDCF()
// Stage 2 (Y6–10): Growth rate + margin fade linearly to terminal
// Stage 3 (Y11+): Gordon Growth terminal value on Year 10 FCFE
// ============================================================

/**
 * Three-stage DCF using FCFE approach.
 * Extends the base model with a 5-year transition period (Y6–Y10)
 * so high-growth companies are not abruptly cut to terminal growth.
 */
export function calculateDCF3Stage(inputs: DCFFCFEInputs): ValuationResult {
  ...
  // ── Shared setup (same as calculateDCF) ──────────────────────────
  const sorted = [...historicals]
    .filter((f) => f.revenue > 0)
    .sort((a, b) => a.fiscal_year - b.fiscal_year);
  ...
  // ── Stage 1 revenue projection (Y1–5, analyst-driven) ────────────
  const stage1Rev = projectRevenue(historicals, estimates, 5);

  // ── Build Stage 1 projections (Y1–5) ─────────────────────────────
  const ke = costOfEquity;
  const g = termGrowth;
  ...
  // ── Stage 2 projections (Y6–10): linear fade ─────────────────────
  const year5 = projections[4];
  const year5GrowthRate = stage1Rev.growthRates[4]; // last analyst-driven growth rate
  const year5NetMargin = year5.net_margin;

  for (let i = 0; i < 5; i++) {
    const stageProgress = i / 4; // 0 → 1 over years 6–10
    ...
    const netIncome = revenue * netMargin;
    const revenueIncrease = Math.max(0, revenue - prevRevenue);
    const yearIdx = 5 + i; // overall projection index (0-based)
  ...
  // ── Terminal value on Year 10 FCFE ────────────────────────────────
  ...
  // ── Sensitivity matrix (same structure as base model) ────────────
  ...
  return {
    ...
    assumptions: {
      revenue_growth_rates: stage1Rev.growthRates.map((r) => Math.round(r * 10000) / 100),
      revenue_source: stage1Rev.source,
      net_margin: Math.round(avgNetMargin * 10000) / 100,
      net_margins_by_year: projections.map((p) => Math.round(p.net_margin * 10000) / 100),
      margin_source: analystMargins.size > 0 ? "analyst" : "historical",
      maintenance_capex: Math.round(maintenanceCapex / 1e6),
