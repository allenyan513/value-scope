...
describe("computeHistoricalMultiples", () => {
  ...
  it("should compute P/E, P/S, P/B for each price point", () => {
...
describe("computeMultiplesStats", () => {
  it("should compute avg, p25, p75, percentile", () => {
    const data = generateHistoricalMultiples(500, 25, 8, 10);
  ...
  it("should return null when fewer than 10 data points", () => {
    const data = generateHistoricalMultiples(5, 25, 8, 10);
    const stats = computeMultiplesStats(data);
  ...
  it("should filter out values above cap", () => {
    const data = generateHistoricalMultiples(200, 25, 8, 10);
    // Inject extreme values
    data[0] = { ...data[0], pe: 500, ps: 200, pb: 100 };
    data[1] = { ...data[1], pe: -10, ps: -5, pb: -3 };
...
describe("computeHistoricalValuations", () => {
  it("should compute fair value as avg × metric", () => {
    const data = generateHistoricalMultiples(500, 25, 8, 10);
    const stats = computeMultiplesStats(data);
    const fin = appleFinancials[0];
    const shares = fin.shares_outstanding;

    const vals = computeHistoricalValuations(stats, fin, shares);

    expect(vals).toHaveLength(3); // PE, PS, PB

    const pe = vals.find((v) => v.type === "pe")!;
    expect(pe.fairValue).toBeCloseTo(pe.historicalAvg * pe.currentMetric, 0);
    expect(pe.metricLabel).toBe("EPS");

    const ps = vals.find((v) => v.type === "ps")!;
    expect(ps.metricLabel).toBe("Revenue/Share");

    const pb = vals.find((v) => v.type === "pb")!;
    expect(pb.metricLabel).toBe("Book Value/Share");
  });

  it("should compute low from p25 and high from p75", () => {
    const data = generateHistoricalMultiples(500, 25, 8, 10);
    const stats = computeMultiplesStats(data);
    const fin = appleFinancials[0];

    const vals = computeHistoricalValuations(stats, fin, fin.shares_outstanding);
    const pe = vals.find((v) => v.type === "pe")!;

    expect(pe.lowEstimate).toBeLessThan(pe.fairValue);
    expect(pe.highEstimate).toBeGreaterThan(pe.fairValue);
  });
  ...
  it("should compute deviation percentage", () => {
    ...
    if (pe.currentMultiple !== null) {
      const expectedDev = Math.round(
        ((pe.currentMultiple - pe.historicalAvg) / pe.historicalAvg) * 100
      );
      expect(pe.deviation).toBe(expectedDev);
    }
  });
  ...
  it("should skip metrics with zero values", () => {
    const data = generateHistoricalMultiples(500, 25, 8, 10);
    const stats = computeMultiplesStats(data);
    const fin = { ...appleFinancials[0], eps: 0, eps_diluted: 0 };

    const vals = computeHistoricalValuations(stats, fin, fin.shares_outstanding);
    // PE should be skipped since EPS = 0
    expect(vals.find((v) => v.type === "pe")).toBeUndefined();
  });
});

describe("sampleData", () => {
  it("should return original data when under limit", () => {
    const data = generateHistoricalMultiples(50, 25, 8, 10);
    const sampled = sampleData(data, 250);
    expect(sampled).toHaveLength(50);
  });

  it("should downsample to target size", () => {
    const data = generateHistoricalMultiples(1000, 25, 8, 10);
    const sampled = sampleData(data, 250);
